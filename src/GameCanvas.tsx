...
const FLAP_ADDRESS = process.env
    .NEXT_PUBLIC_FLAP_TOKEN_ADDRESS as `0x${string}`;
const BACKEND_WALLET = process.env.NEXT_PUBLIC_BACKEND_ADDRESS as `0x${string}`;
const SKILL_NFT_ADDRESS = process.env
    .NEXT_PUBLIC_SKILL_NFT_ADDRESS as `0x${string}`;
const SKILL_MARKETPLACE_ADDRESS = process.env
    .NEXT_PUBLIC_SKILL_NFT_MARKETPLACE_ADDRESS as `0x${string}`;
...
export default function GameCanvas() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (flappymons.length > 0) {
                ...
                console.log(
                    "[Auto-Equip] First Flappymon selected:",
                    flappymons[0]
                );
            }
        })();
    ...
    // useEffect(() => {
    //     // expose the router to Phaser
    //     window.__gotoReactPage = (path: string) => {
    //         router.push(path);
    //     };
    // }, []);
        ...
        <>
            <div
                className={`w-screen h-screen fixed z-10 ${
                    address ? "hidden" : "flex"
                } justify-center items-center bg-slate-900`}
            >
                <WalletOptions />
            </div>
